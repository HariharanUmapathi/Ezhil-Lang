...
class EzhilWeb():
    ...
    def __init__(self,debug = False):
        ...
        if ( self.debug ):
            # debugging tips
            import cgitb
            cgitb.enable(display=1, logdir="/tmp/cgi-errors")
        ...
        if ( self.debug ):
            print("<br>Before do_ezhil_execute",str(program))
...
<script lang="text/javascript">
  function download(filename, content) {
    saveAs( new Blob([content],{type: "application/x-ezhil;charset=utf-8"}), filename);
        ...
        print("Protram ",program)
        try:
            failed = False
            print("Protram 2",program)
            obj = EzhilFileExecuter( file_input = [program],debug=True, redirectop = True, TIMEOUT = 60*2,encoding='utf-8', ) # 2 minutes
            obj.run()
...
if __name__ == '__main__':
    ...
    #response = subprocess.Popen(["python3", "-m" ,"ezhil","-stdin"],input=cgi.FieldStorage())
    #print(cgi.FieldStorage())
    # do the Ezhil thing
    #EzhilWeb(debug=True)
    # Read CGI POST bytes
    program = cgi.FieldStorage().getvalue("prog")
    print("Your Input:",program)
    subprocess.run(["python3","-m","ezhil","-stdin"],input=program.encode("utf-8"))
